...
!
      ...
      integer i,nc,itst,lmax,l,ipos,iq,nh,nreadpts,nk,k
!!$!
...
! Read in free boundary
      nh=39
      open(unit=nh,file='hidelta04_rob4_1.dat', &
...
!*************************************************
! Dmitri's input file from TOKAMEQ free boundary code
      open(unit=nh,file='STEP15.TXT', &
           status='unknown',iostat=ios)
      if(ios.ne.0) then
         write(6,*) 'problem opening STEP15.TXT'
         stop
      endif
      do i=1,23
        read(nh,*)string1
      ...
      do i=1,57
        read(nh,*)zdim(i+1),rdim(i+1),rdim(117-i)
      end do
      close(nh)
      zdim(59)=-zdim(1)
      rdim(59)=rdim(1)
      do i=1,57
        zdim(59+i)=zdim(58-i+1)
      end do
      nreadpts=116
!*************end Dmitrij******************************
      qaxis=sfac(ncon)
